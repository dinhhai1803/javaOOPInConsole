...
public class CheckInput {
    ...
    public static int checkBirthDate(String msg){
        ...
        do{
            try{
                n = Integer.parseInt(Inputter.inputSrt(msg));
                flag = false;
            }catch(Exception e){
                System.out.println("Invalid year");
            }
        }while((flag != true) || (n <= 1900) || (n > 2021))  ;
        return n;
    }

    public static long checkPhone(String msg){
        boolean flag = true;
        long n = 0;
        do{
            try{
        ...
        }while((flag != true) || (n <= 999999999))  ;
        return n ;
    }

    public static int checkYearOfExp(String msg){
        boolean flag = true;
        int n = 0;
        do{
            try{
        ...
        }while((flag != true) || (n <= 0) || (n > 100))  ;
        return n;
    }

    public static String checkRank(String msg){
        
        boolean flag = true;
        String rank = "";
        String[] validRank = {"EXCELLENCE", "GOOD", "FAIR", "POOR"};
        while(flag == true){
            rank = Inputter.inputSrt(msg);
            for(int i=0; i<validRank.length; i++){
                if(rank.toUpperCase().equals(validRank[i])){
                    flag = false;
                }
            }
        }
